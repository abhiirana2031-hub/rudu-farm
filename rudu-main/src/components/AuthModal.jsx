...
export const AuthModal = () => {
  const { currentUser, isAuthModalOpen, setIsAuthModalOpen, loginUser, currentRole, setCurrentRole } = useFarm();
  ...
  const handleLoginSubmit = (e) => {
    e.preventDefault();
    if (!email || !password) {
      setErrorMsg(selectedRole === 'farmer' ? 'Enter your Farmer ID/Mobile and Password.' : 'Enter your Email and Password.');
      return;
    }
    setErrorMsg('');
    let displayName = email.split('@')[0] || 'User';
    if (selectedRole === 'farmer' && email.toLowerCase() === 'rf1024') displayName = 'Ramesh Yadav';
    else if (selectedRole === 'employee' && email.toLowerCase().includes('amit')) displayName = 'Amit Kumar';
    else if (selectedRole === 'admin' && email.toLowerCase().includes('rajesh')) displayName = 'Rajesh Sharma';
    loginUser({ name: displayName, email: email.includes('@') ? email : `${email}@rudufarm.com`, role: selectedRole });
  };
  ...
  const handleQuickDemo = (role) => {
    setErrorMsg('');
    const demos = {
      admin: { name: 'Rajesh Sharma', email: 'admin@rudufarm.com', role: 'admin' },
      employee: { name: 'Amit Kumar', email: 'amit@rudufarm.com', role: 'employee' },
      farmer: { name: 'Ramesh Yadav', email: 'ramesh@rudufarm.com', role: 'farmer', village: 'Rampur', cattleCount: 5, memberCategory: 'Gold Member' },
    };
    loginUser(demos[role]);
  };

  const roleConfig = {
    farmer:   { placeholder: 'Farmer ID or Mobile (e.g. RF1024)', btn: 'Access Passbook',      demo: 'Ramesh Yadav — Farmer' },
    employee: { placeholder: 'Operator Email or Agent ID',         btn: 'Open Agent Hub',        demo: 'Amit Kumar — Operator' },
    admin:    { placeholder: 'Administrator Email',                 btn: 'Sign In to Admin',      demo: 'Rajesh Sharma — Admin' },
  };
    ...
    <div className="auth-modal-overlay">
      <div className="auth-modal-card auth-modal-card-illustrated" style={{ maxWidth: '420px' }}>
        ...
        <div style={{ padding: '16px 18px', background: 'transparent' }}>
          ...
          {/* Quick Demo */}
          <div style={{ marginTop: '14px', padding: '12px 14px', background: 'rgba(0, 0, 0, 0.35)', backdropFilter: 'blur(10px)', borderRadius: '16px', border: '1px solid rgba(255, 255, 255, 0.22)' }}>
            <p style={{ fontSize: '11px', color: '#F6E05E', fontWeight: '900', textAlign: 'center', marginBottom: '8px', textTransform: 'uppercase', letterSpacing: '0.8px', textShadow: '0 1px 3px rgba(0,0,0,0.8)' }}>
              ⚡ Quick Demo
            </p>
            <button
              type="button"
              ...
              style={{
                ...
                border: '1.5px solid rgba(255,255,255,0.9)', background: '#FFFFFF',
                color: '#1B5E20', fontWeight: '900', fontSize: '13px',
          ...
          {/* Direct Links Footnote */}
          <div style={{ marginTop: '10px', fontSize: '10.5px', color: 'rgba(255,255,255,0.85)', textAlign: 'center', fontWeight: '700', textShadow: '0 1px 3px rgba(0,0,0,0.8)' }}>
            Direct Access: <a href="#admin" onClick={() => handleQuickDemo('admin')} style={{ color: '#F6E05E', fontWeight: '900', textDecoration: 'underline' }}>/admin</a> (Admin) &bull; <a href="#bmc" onClick={() => handleQuickDemo('employee')} style={{ color: '#F6E05E', fontWeight: '900', textDecoration: 'underline' }}>/bmc</a> (Operator)
          </div>
