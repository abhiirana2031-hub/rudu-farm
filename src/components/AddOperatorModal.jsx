...
export const AddOperatorModal = () => {
  const { activeModal, setActiveModal, addEmployee, collectionCenters } = useFarm();

  const [name, setName] = useState('');
  const [phone, setPhone] = useState('');
  const [center, setCenter] = useState(collectionCenters?.[0]?.name || '');
            ...
            <div className="form-group">
              ...
              >
                {collectionCenters && collectionCenters.length > 0
                  ...
                  : <option value="">No centres yet — add one first</option>
                }
              </select>
            </div>
