...
export const AdminDashboard = ({ setActiveTab }) => {
  const { farmers, entries, payouts, setActiveModal, openSlip, sessions, currentUser } = useFarm();
      ...
      <div className="farmer-profile-card-v2">
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
            ...
            <div style={{ textAlign: 'left' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flexWrap: 'wrap' }}>
                <h2 style={{ fontSize: '22px', fontWeight: '800', margin: 0, color: '#3B2214' }}>{currentUser?.name || 'Admin'}</h2>
              ...
              <div style={{ fontSize: '12px', color: '#4E2A18', display: 'flex', alignItems: 'center', gap: '4px', marginTop: '4px', fontWeight: '700' }}>
                <MapPin size={13} /> Role: <span>{currentUser?.designation || 'Dairy Owner / Manager'}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="stats-row">
          ...
          <div className="stat-col">
            ...
            <span className="stat-val">{activeSessions.length > 0 ? `${activeSessions.length} Live` : 'No Active'}</span>
          </div>
        </div>
      </div>
      ...
      <div>
        ...
        <div style={{ background: '#FFFFFF', border: '1.5px solid #EFE2D5', borderRadius: '24px', padding: '20px', marginBottom: '24px', textAlign: 'left' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
            <div>
              <h3 style={{ fontSize: '16px', fontWeight: '800', color: '#3B2214', margin: 0 }}>OPERATOR SESSIONS</h3>
              <div style={{ fontSize: '13px', color: '#166534', fontWeight: '700', marginTop: '4px', display: 'flex', alignItems: 'center', gap: '4px' }}>
                🟢 {activeSessions.length} Active Operator{activeSessions.length !== 1 ? 's' : ''}
              </div>
            </div>
            <button onClick={() => setActiveTab('operator-sessions')} className="btn btn-secondary btn-sm" style={{ padding: '6px 12px' }}>
              View All Sessions
            </button>
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            {activeSessions.length === 0 ? (
              <div style={{ padding: '16px', textAlign: 'center', color: 'var(--text-muted)', fontSize: '13px', fontStyle: 'italic', background: '#F8FAFC', borderRadius: '12px' }}>
                No operators currently logged in.
              </div>
            ) : (
              activeSessions.map(s => (
                <div key={s.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 16px', background: '#F8FAFC', borderRadius: '12px', border: '1px solid #E2E8F0' }}>
                  <div>
                    <div style={{ fontWeight: '700', color: '#1E293B', fontSize: '14px' }}>{s.operatorName}</div>
                    <div style={{ fontSize: '11px', color: '#64748B', marginTop: '2px' }}>
                      {s.shift} Session ({s.actualLogin} – {s.scheduledEnd})
                    </div>
                  </div>
                  <div style={{ textAlign: 'right' }}>
                    <div style={{ fontWeight: '800', color: '#4E2A18', fontSize: '14px' }}>{s.entriesCount} entries</div>
                    <div style={{ fontSize: '11px', color: '#10B981', fontWeight: '700', marginTop: '2px' }}>{s.volumeLogged} L logged</div>
                  </div>
                </div>
              ))
            )}
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
            ...
            <div style={{ textAlign: 'left' }}>
              ...
              <div style={{ fontSize: '12px', color: '#7C695D', fontWeight: '600' }}>
                Plant Manager: {currentUser?.name || 'Admin'}
              </div>
              <div style={{ fontSize: '11px', color: '#AB988B', marginTop: '2px' }}>
                Active Sessions: {activeSessions.length} | Active Farmers: {activeFarmersCount} | Daily Volume: {todayVolume.toLocaleString()}L
              </div>
            </div>
          </div>
