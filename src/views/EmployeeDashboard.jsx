...
export const EmployeeDashboard = () => {
  const { farmers, entries, addMilkEntry, openSlip, calculateRate, currentUser } = useFarm();
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
            ...
            <div style={{ textAlign: 'left' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flexWrap: 'wrap' }}>
                <h2 style={{ fontSize: '22px', fontWeight: '800', margin: 0, color: '#3B2214' }}>{currentUser?.name || 'Operator'}</h2>
              ...
              <div style={{ fontSize: '12px', color: '#4E2A18', display: 'flex', alignItems: 'center', gap: '4px', marginTop: '4px', fontWeight: '700' }}>
                <MapPin size={13} /> Role: <span>{currentUser?.designation || 'Milk Operator'}</span>
              </div>
            </div>
          </div>
        </div>
                ...
                todayEntries.map(e => (
                  <div key={e.id} style={{ background: '#FFFFFF', border: '1px solid var(--border)', borderRadius: '16px', padding: '16px', boxShadow: 'var(--shadow-sm)' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '12px' }}>
                      <div>
                        <strong style={{ color: '#3B2214', fontSize: '14px' }}>{e.timestamp}</strong>
                        <div style={{ fontSize: '11px', color: '#AB988B' }}>ID: {e.id}</div>
                      </div>
                      <div style={{ textAlign: 'right' }}>
                        <span style={{ fontWeight: '800', color: '#3B2214', fontSize: '14px' }}>{e.farmerName}</span>
                    ...
                    <button onClick={() => openSlip(e)} className="btn btn-secondary btn-sm" style={{ width: '100%', justifyContent: 'center', padding: '8px', background: '#EAF4EE', color: '#4E2A18', border: '1px solid #DCC5B3' }}>
